...
class MuzzleCanvas {
  ...
  async basic(x, y, imagePath) {
    ...
    this._config('shuffler', headbreaker.Shuffler.grid);
  ...
  async choose(leftUrl, rightUrl, leftOddUrls, rightAspectRatio) {
    this._config('shuffler', headbreaker.Shuffler.line);
  ...
  async match(leftUrls, rightUrls, {leftOddUrls = [], rightOddUrls = [], rightAspectRatio = this.effectiveAspectRatio} = {}) {
    ...
    this._config('shuffler', headbreaker.Shuffler.columns);
  ...
  _config(key, value) {
    ...
    console.debug(`Configuring ${key}. Current value is ${current}`)
