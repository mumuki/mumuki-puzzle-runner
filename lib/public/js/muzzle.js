 ...
 * @property {Point[]} positons list of points
 */
...
const Muzzle = {
   ...
   */
  expectedRefsAreOnlyDescriptive: false,
  ...
  basic(x, y, imagePath) {
     ...
     **/
    this._loadImage(imagePath).then((image) => {
      /** @type {Canvas} */
      // @ts-ignore
  ...
  match(lefts, rights) {
    ...
    for (let i = 0; i < lefts.length; i++) {
      pieces.push(
        this._loadImage(lefts[i].imagePath).then((image) =>({
          structure: lefts[i].structure || 'TTNS',
          metadata: {image}
        })));
      pieces.push(
        this._loadImage(rights[i].imagePath).then((image) =>({
          structure: rights[i].structure || 'NTSS',
          metadata: {image}
        })));
    }
    return Promise.all(pieces).then(pieces  => {
      /** @type {Canvas} */
      const canvas = new headbreaker.Canvas(this.canvasId, this._canvasConfig(null));
      pieces.forEach(it => canvas.sketchPiece(it));
      this._attachValidator(canvas);
      this._configInitialCanvas(canvas);
      return canvas;
    });
