...
class MuzzleCanvas {
  ...
  async match(lefts, rights, leftOdds, rightOdds) {
    ...
    leftOdds.forEach(it => {
      templatePromises.push(this._buildTemplate(it, `T-N-`, {odd: true}));
    })

    rightOdds.forEach(it => {
      templatePromises.push(this._buildTemplate(it, `N-S-`, {odd: true}));
    })
   ...
   */
  _buildTemplate(config, structure, {odd = false} = {}) {
