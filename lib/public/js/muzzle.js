...
class MuzzleCanvas {
  ...
  async match(lefts, rights, leftOdds = [], rightOdds = []) {
    ...
    for (let i = 0; i <= last; i++) {
      ...
      pushTemplate(lefts[i], {id: leftId, left: true, rightTargetId: rightId});
      pushTemplate(rights[i], {id: rightId});
    }

    leftOdds.forEach((it, i) =>  pushTemplate(it, {id: `lo${i}`, left: true, odd: true}));
    rightOdds.forEach((it, i) => pushTemplate(it, {id: `ro${i}`, odd: true}));
   ...
   */
  _createTemplate(config, {id, left = false, rightTargetId = null, odd = false}) {
