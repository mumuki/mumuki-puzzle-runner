...
const Muzzle = {
  ...
  dumpPreviousSolution() {
    this.previousSolutionJson = JSON.stringify(this.solution);
  },
  ...
  submit() {
    ...
    this.dumpPreviousSolution();
    // @ts-ignore
    new mumuki.bridge.Laboratory()._submitSolution(this._bridgeRequest);
  },

  /**
   * The request object that can be sent to
   * the laboratory bridge
   */
  get _bridgeRequest() {
    return {
      solution: { content: JSON.stringify(this.solution) },
      /**@todo result: this.currentCanvas.valid*/
    }
  }
}
